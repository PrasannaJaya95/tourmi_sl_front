/**
 * Rental period math shared by contracts, invoices, and agreements.
 * Keep in sync with back/src/lib/rentalPeriod.js
 */
...
function isMidnightTime(timeStr) {
    const p = parseTimeTo24h(timeStr);
    return Boolean(p && p.h === 0 && p.min === 0);
}

function isEndOfDayTime(timeStr) {
    const p = parseTimeTo24h(timeStr);
...
function getAgreementToDate(pickupDate, pickupTime, dropoffDate, dropoffTime) {
...
function getAgreementRentalDays(pickupDate, pickupTime, dropoffDate, dropoffTime) {
    const units = computeRentalDayUnits(pickupDate, pickupTime, dropoffDate, dropoffTime);
